...
- [New Language Features](#new-language-features)
  ...
  - [TestTag](#testtag)
  - Deprecated
    - [Friend](#friend) replace with [CallableFrom](#callableFrom)
...
A method or class can supply via a `#[CallableFrom]` attribute a list of classes. Only these classes can call the method.
...
**NOTES:**
- Multiple classes can be specified. E.g. `#[CallableFrom(Foo::class, Bar::class)]`
- A class can have a `#[CallableFrom]` attribute, classes listed here are applied to every method.
  ```php 
  #[CallableFrom(Foo::class)]
  class Entity
  {
    public function ping(): void // ping can be called from Bar
    {
    }
  }
  ```
- The `#[CallableFrom]` attribute is additive. If a class and a method have the `#[CallableFrom]` the method can be called from any of the classes listed. E.g. 
  ```php 
  #[CallableFrom(Foo::class)]
  class Entity
  {
    #[CallableFrom(Bar::class)] 
    public function pong(): void // pong can be called from Foo and Bar
    {
    }
  }
  ```  
...
This has been renamed. Update your code to use `#[CallableFrom]`.

The attribute was initially called  `#[Friend]` because it was loosley based on the C++ friend feature. 
